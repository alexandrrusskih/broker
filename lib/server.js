...
// DROP AFTER 2026-12. An installation from before the September 2026 rename
// keeps its data where it already is: this default only applies when nothing has
// been created yet, and live token stores are not worth moving for a tidier name.
// An explicit --data-dir (and the path recorded in service.json) always wins.
const LEGACY_DATA_DIR = path.join(os.homedir(), ".local", "share", "hltm-broker");
const DEFAULT_DATA_DIR = require("node:fs").existsSync(LEGACY_DATA_DIR)
  ? LEGACY_DATA_DIR
  : path.join(os.homedir(), ".local", "share", "broker");
