const { providers, secretsFor, codexHandle, handleMatches, isDeadGrant, createShared } = require("./shared");

function createOAuthHandlers(store, { logger = console } = {}) {
  const { log, error } = logger;
  const { alertOnce } = createShared(store);
  ...
  return { oauthRefresh, oauthRevoke };
}

module.exports = { createOAuthHandlers };
