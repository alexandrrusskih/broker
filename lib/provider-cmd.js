...
async function install(provider, flags) {
  ...
  const say = flags.quiet ? () => {} : console.log;
  const r = installWrapper(provider, null, flags["bin-dir"], { container: flags.container === true });
  say(`✓ wrapper '${r.cmd}' → ${r.path}`);
  for (const old of r.dropped || []) say(`  removed the old name ${old}`);

  if (flags["no-shim"]) {
    say(`  shim skipped — plain '${w.bin}' still bypasses the broker`);
  } else {
    const s = shim.install(provider, flags["bin-dir"]);
    say(`✓ '${w.bin}' now goes through ${r.cmd} (${s.target})`);
  ...
  if (chosen) say(`✓ your account: ${chosen} (stays on it while it has room)`);

  if (!r.inPath) say(`  note: ${path.dirname(r.path)} is not in PATH — add it`);
  say(`\n  ${w.bin} …           runs through the broker now`);
  say(`  ${r.cmd} list        who is seeded and what is left`);
  say(`  broker ${provider} remove   undo the shim`);
}
