...
const { WRAP, binDirFor, pkgDirFor } = require("./wrap");
...
//
// Shims written before the September 2026 rename say "hltm-broker shim", which
// CONTAINS this string — so they keep being recognised with no second marker to
// carry around. Never shorten this to "broker" alone: it would then match any
// file that merely mentions the word.
const MARK = "broker shim";
...
function stashDir(binDir) {
  return path.join(pkgDirFor(binDirFor(binDir)), "real");
}
...
// A harness that updates itself renames the running program aside — agy writes
// `agy.<stamp>.old` next to it — and never comes back for the copy. Since the
// shim moved that program into the broker's own directory, every update left
// another 180 MB there. Cleaned up on install, which is what runs after an
// update on every path: `broker upgrade --all` and `broker-agy upgrade` alike.
// Only files the updater itself could have named are touched: same directory,
// same base name, a numeric stamp, `.old`.
function dropUpdateLeftovers(provider, binDir) {
  const name = WRAP[provider].bin;
  const dir = stashDir(binDir);
  const leftover = new RegExp(`^${name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\.\\d+\\.old$`);
  let dropped = [];
  try {
    dropped = fs.readdirSync(dir).filter((entry) => leftover.test(entry));
    for (const entry of dropped) fs.rmSync(path.join(dir, entry), { force: true });
  } catch (_e) {
    // no stash directory for this provider, or not ours to clean
  }
  return dropped;
}

// After a harness updates itself, point at what it just installed.
//
// The wrapper runs the version recorded in shim_previous, so that `install
// stable` is not quietly overridden by whatever `latest` left on disk. But an
// updater installs beside the old version and leaves the launcher alone — ours
// is not its launcher — so nothing moved that record, and `broker upgrade`
// updated a harness that then kept starting the old one. Verified on this
// machine: 2.1.257 and 2.1.267 installed, 2.1.236 still running.
function adoptNewestVersion(provider) {
  const where = WRAP[provider]?.versions;
  if (!where) return null;
  const dir = path.join(os.homedir(), where);
  let installed = [];
  try {
    installed = fs.readdirSync(dir).filter((name) => /^\d+\.\d+\.\d+/.test(name));
  } catch (_e) {
    return null;
  }
  const order = (v) => v.split(".").map((part) => parseInt(part, 10) || 0);
  ...
  const target = path.join(dir, newest);
  const cfg = config.read();
  const previous = { ...(cfg.shim_previous || {}) };
  if (previous[provider] === target) return null;
  // Only a version this updater manages: a stashed binary or a symlink
  // elsewhere is someone else's arrangement and stays untouched.
  if (previous[provider] && !previous[provider].startsWith(dir + path.sep)) return null;
  previous[provider] = target;
  config.write({ shim_previous: previous });
  return { provider, version: newest, path: target };
}
...
exec ${wrapper} "$@"
`;
  const tmp = `${target}.broker-${process.pid}`;
  ...
  return { target, wrapper, cleaned: dropUpdateLeftovers(provider, binDir) };
}
...
// Must match what the engine names that directory (run.py: "broker-real-%s-%d").
const SHIM_FREE = /broker-real-[a-z]+-\d+$/;
...
module.exports = { install, remove, ensure, status, isShim, shimPath, dropUpdateLeftovers, adoptNewestVersion, MARK };
