const { providers, secretsFor, codexHandle, isDeadGrant, createShared } = require("./shared");

function createTokenHandlers(store, { logger = console, handlesOnly = false } = {}) {
  const { log, error } = logger;
  const { authorized, alertOnce } = createShared(store);
  ...
  async function getToken(req, res) {
    ...
    const p = Object.hasOwn(providers, provider) && providers[provider];
    ...
    try {
      ...
      if (format === "authjson") {
        if (provider === "codex") {
          ...
          const authFile = handlesOnly || cfg.codex_handle_rollout === true
  ...
  async function listAccounts(req, res) {
    ...
    const provider = String(req.query.provider || "");
    if (!Object.hasOwn(providers, provider)) return res.status(400).json({ error: `unknown provider: ${provider}` });
  ...
  async function seedToken(req, res) {
    if (!(await authorized(req, true))) return res.status(401).json({ error: "unauthorized" });
    if (req.method !== "POST") return res.status(405).json({ error: "use POST" });

    const { provider, account = "default", ...fields } = req.body || {};
    if (!Object.hasOwn(providers, provider)) return res.status(400).json({ error: `unknown provider: ${provider}` });
    if (typeof account !== "string" || !account) return res.status(400).json({ error: "account required" });
  ...
  return { getToken, listAccounts, seedToken, deleteAccount };
}

module.exports = { createTokenHandlers };
