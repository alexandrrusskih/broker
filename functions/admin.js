...
function createAdminHandlers(store, { logger = console, handlesOnly = false } = {}) {
  const { log } = logger;
  const { authorized } = createShared(store);
  ...
  async function bootstrap(req, res) {
    ...
    const claimed = await store.claimConfig({
  ...
  async function configSet(req, res) {
    ...
    const type = Object.hasOwn(CONFIG_SETTABLE, key) && CONFIG_SETTABLE[key];
    if (!type) {
    ...
    if (handlesOnly && key === "codex_handle_rollout" && !value) {
      return res.status(400).json({ error: "self-hosted Codex always uses refresh handles" });
    }
    await store.writeConfig({ [key]: value });
  ...
  async function configGet(req, res) {
    ...
    return res.status(200).json({
      codex_handle_rollout: handlesOnly || cfg.codex_handle_rollout === true,
      codex_handle_secret_set: Boolean(cfg.codex_handle_secret)
    });
  }

  return { bootstrap, configSet, configGet };
}
