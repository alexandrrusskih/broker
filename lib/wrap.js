...
function refreshContainerOverlay(provider, engineDir, create = false,
  ...
  fs.rmSync(path.join(overlayRoot, "home", ".local", "lib", "hltm-broker"), {
    recursive: true,
    force: true
  });
...
function installContainer(provider, overlayRoot) {
  ...
  const staging = fs.mkdtempSync(path.join(os.tmpdir(), "hltm-container-"));
  try {
...
module.exports = { install, installContainer, binDirFor, WRAP };
