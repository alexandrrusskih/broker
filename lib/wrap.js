...
// DROP AFTER 2026-12. The engine directory was ~/.local/lib/hltm-broker until
// September 2026, and `real/` under it holds binaries the shims moved aside —
// including agy, which IS its 178 MB program and exists nowhere else. Renaming
// the directory without carrying that across would leave the harness unfindable.
function claimLegacyEngineDir(root) {
...
function install(provider, account, binDirOverride, options = {}) {
  ...
  const script = w.template
    ? templateWrapper(provider, w, binDirOverride ? pkgDirFor(binDirOverride) : null, options.container === true)
...
module.exports = { install, installContainer, binDirFor, pkgDirFor, WRAP };
